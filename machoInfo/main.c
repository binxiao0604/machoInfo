        ...
        while (left > 0) {
          ...
          if (left > 0) {
            i++;
              ...
              size_t NullPos = strlen(strrchr(dest, '\0'));
  ...
  for (uint32_t i = 0U; i < ncmds; i++) {
    ...
    if (cmd->cmd == LC_SEGMENT_64) {
      ...
      if (is_swap) {
        ...
        printf("LC: %d segname: %s vmaddr: %llx nsects: %x\n", i, segment->segname, segment->vmaddr, segment->nsects);
      ...
      if (is_swap) {
        ...
        printf("LC: %d segname: %s vmaddr: %x\n",i, segment->segname, segment->vmaddr);
